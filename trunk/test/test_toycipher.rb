...
class TestRot13 < Test::Unit::TestCase
  ...
  def test_rotate_alphabet
    ...
    assert_equal 'Z', @tc.alph.first, "boo: #{@tc.alph.first}"
    assert_equal 'A', @tc.alph[1], 'shala'
    assert_equal 'Y', @tc.alph.last, 'ka'
  ...
  def test_one_time_pad
    ...
    key = 'abc'
    ciphertext = 'ACEACE'
    assert_equal @tc.xor(plaintext, key), ciphertext
  end
